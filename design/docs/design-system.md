...
| `--radius-xl`   | `16px`   | `rounded-xl`   | 容器、Hero 区域    |
| `--radius-2xl`  | `20px`   | `rounded-2xl`  | 弹窗            |
...
3. 弹窗（Modal/Drawer）使用 `rounded-2xl`（20px）。
...
4. **主按钮使用** **`shadow-brand`**，品牌色阴影增强品牌感。
...
- **侧边栏变 Drawer**：在 `lg`（1024px）以下，侧边栏变为抽屉式，带遮罩层 (overlay)

- **遮罩层**：`z-index: 1200`（`--z-overlay`），半透明黑色背景

- **Header 保持**：Header 在所有断点保持 64px 高度

- **内容内边距缩减**：移动端内容区使用 `p-4`（16px），桌面端使用 `p-6`（24px）
...
4. **内容区内边距**：桌面端 `p-6`（24px），移动端 `p-4`（16px）。
...
以下 TypeScript 对象将设计令牌映射到 Naive UI 的主题系统。在应用入口（如 `App.vue` 或 `main.ts`）中通过 `<n-config-provider :theme-overrides="themeOverrides">` 注入。
...
export const themeOverrides: GlobalThemeOverrides = {
  common: {
    ...
    primaryColorSuppl: '#5B5FE9',     // primary-500

    // ---- 成功色 ----
    successColor: '#22B570',          // success-500
    successColorHover: '#1A965C',     // success-600
    successColorPressed: '#0F7A48',   // success-700
    successColorSuppl: '#22B570',     // success-500

    // ---- 错误色 ----
    errorColor: '#F0503C',            // error-500
    errorColorHover: '#D63A28',       // error-600
    errorColorPressed: '#B22A1A',     // error-700
    errorColorSuppl: '#F0503C',       // error-500

    // ---- 警告色 ----
    warningColor: '#FFA42B',          // warning-500
    warningColorHover: '#E68A00',     // warning-600
    warningColorPressed: '#B86D00',   // warning-700
    warningColorSuppl: '#FFA42B',     // warning-500

    // ---- 信息色 ----
    infoColor: '#3B8BFF',             // info-500
    infoColorHover: '#1E6FE0',        // info-600
    infoColorPressed: '#1E6FE0',      // info-600
    infoColorSuppl: '#3B8BFF',        // info-500

    // ---- 文字色 ----
    textColorBase: '#242428',         // neutral-900
    textColor1: '#242428',            // text-primary   — 主要文字
    textColor2: '#6B6B76',            // text-secondary — 次要文字
    textColor3: '#8A8A96',            // text-tertiary  — 辅助文字
    textColorDisabled: '#B0B0BC',     // text-disabled
    placeholderColor: '#8A8A96',      // text-tertiary
    iconColor: '#6B6B76',             // neutral-600
    iconColorHover: '#5B5FE9',        // primary-500

    // ---- 背景色 ----
    bodyColor: '#F4F4F6',             // bg-page — 页面背景
    cardColor: '#FFFFFF',             // bg-card
    modalColor: '#FFFFFF',            // bg-card
    popoverColor: '#FFFFFF',          // bg-card
    tableColor: '#FFFFFF',            // bg-card
    inputColor: '#FFFFFF',            // bg-card
    inputColorDisabled: '#FAFAFB',    // bg-subtle
    actionColor: '#FAFAFB',           // bg-subtle
    hoverColor: '#F4F4F6',            // bg-hover
    tableColorHover: '#FAFAFB',       // bg-subtle
    tableColor striped: '#FAFAFB',    // bg-subtle

    // ---- 边框色 ----
    borderColor: '#E8E8EC',           // border-default — neutral-200
    dividerColor: '#E8E8EC',          // border-default
    tableBorderColor: '#E8E8EC',      // border-default
    ...
    fontSize: '14px',                 // text-base — 正文默认
    fontSizeMini: '12px',             // text-xs
    fontSizeTiny: '12px',             // text-xs
    fontSizeSmall: '13px',            // text-sm
    fontSizeMedium: '14px',           // text-base
    fontSizeLarge: '16px',            // text-lg
    fontSizeHuge: '20px',             // text-xl
  ...
  Button: {
    ...
    heightTiny: '22px',
    // 次要按钮
    textColorSecondary: '#5B5FE9',
    colorSecondary: '#EEF0FF',
    colorHoverSecondary: '#DBDEFF',
    colorPressedSecondary: '#B8BEFF',
    borderSecondary: '1px solid #DBDEFF',
  },
  ...
  Input: {
    ...
    borderRadius: '8px',
    boxShadowFocus: '0 0 0 2px rgba(91, 95, 233, 0.15)',
    fontSizeMedium: '14px',
    fontSizeLarge: '16px',
    fontSizeSmall: '13px',
    heightMedium: '36px',
  ...
  Checkbox: {
    colorChecked: '#5B5FE9',
    colorCheckedHover: '#6E75F5',
    colorCheckedPressed: '#4A48D4',
    colorFocused: '#5B5FE9',
    border: '1px solid #D5D5DC',
    borderHover: '1px solid #5B5FE9',
    borderChecked: '1px solid #5B5FE9',
    borderFocus: '1px solid #5B5FE9',
    boxShadowFocus: '0 0 0 2px rgba(91, 95, 233, 0.15)',
    borderRadius: '4px',
    textColor: '#242428',
  ...
  Menu: {
    color: '#FFFFFF',
    colorActive: '#EEF0FF',
    colorActiveHover: '#DBDEFF',
    colorActiveCollapsed: '#EEF0FF',
  ...
  Modal: {
    ...
    titleFontSize: '20px',
    titleFontWeight: '600',
    titleTextColor: '#242428',
    closeColor: '#8A8A96',
    closeColorHover: '#5B5FE9',
  ...
  Drawer: {
    color: '#FFFFFF',
    textColor: '#242428',
    headerBorderBottom: '1px solid #E8E8EC',
    footerBorderTop: '1px solid #E8E8EC',
    titleFontSize: '20px',
    titleFontWeight: '600',
    bodyPadding: '24px',
  ...
  Switch: {
    railColor: '#E8E8EC',
    railColorActive: '#5B5FE9',
    railColorActiveHover: '#6E75F5',
    buttonColor: '#FFFFFF',
  ...
  Dropdown: {
    color: '#FFFFFF',
    textColor: '#242428',
    optionTextColor: '#242428',
    optionTextColorHover: '#5B5FE9',
    optionColorHover: '#F4F4F6',
    optionColorActive: '#EEF0FF',
    borderRadius: '8px',
    boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)',
    optionFontSize: '14px',
    optionHeight: '36px',
  ...
  Message: {
    ...
    fontSize: '14px',
    padding: '10px 16px',
    iconColorInfo: '#3B8BFF',
    iconColorSuccess: '#22B570',
    iconColorWarning: '#FFA42B',
    iconColorError: '#F0503C',
  },
  ...
  Form: {
    ...
    feedbackTextColorError: '#D63A28',
    feedbackTextColorWarning: '#E68A00',
    labelRequiredMark: '#F0503C',
  },
...
1. 在 `App.vue` 中包裹 `<n-config-provider :theme-overrides="themeOverrides">`。
2. 以上对象为**完整覆盖配置**，所有组件的关键视觉属性均已映射设计令牌。
3. 如需局部主题覆盖（如深色模式），可在子组件层级使用独立的 `<n-config-provider>` 覆盖。
4. `tableColor striped` 属性名中的空格是 Naive UI 的已知命名，实际使用时请确认 Naive UI 版本兼容性。
...
2. 在 `main.ts` 或 `App.vue` 中引入 Naive UI `themeOverrides`（见第 8 节）。
3. 在 `main.ts` 中引入 `design-tokens.css`，使 CSS 变量全局可用。
